...
function createDawgVisualizationWithFabric(dawg) {
    ...
    canvas.on('mouse:wheel', function (opt) {
        ...
        const zoomPoint = fabric.util.transformPoint(point, canvas.viewportTransform);
    ...
    root.descendants().forEach((node, i) => {
        ...
        if (node.parent && node.parent.parent) {
            const line = new fabric.Line([node.x, node.y, node.parent.x, node.parent.y], {
                stroke: 'black',
                hasBorders: false,
                hasControls: false,
                selectable: false
    ...
    const centerX = canvas.width / 2 - root.x;
    const centerY = canvas.height / 2 - root.y;
    canvas.viewportTransform[4] = centerX;
    canvas.viewportTransform[5] = centerY;
}

d3.json("dawg_data.json").then(rawData => {
